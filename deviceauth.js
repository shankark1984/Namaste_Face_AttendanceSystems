...
const fetchDataFromGoogleSheets = async range => {
    try {
        const response = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID_DA}/values/${Device_Details_Range}?key=${API_KEY_DA}`);
        if (!response.ok) {
...
const validateDevice = async deviceId => {
    const data = await fetchDataFromGoogleSheets(deviceId);
    if (!data) {
    ...
    device_NoofDevice = deviceRow[3];
    device_NoofActive = deviceRow[4];
    device_Status = deviceRow[5];

    if (device_ValidDate < currentDate) {
        console.log("Device is expired, kindly conatct to you admin");
        return false;
